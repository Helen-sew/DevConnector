...
router.post(
  ...
  async (req, res) => {
    ...
    if (skills) {
      profileFields.skills = skills.split(',').map((skill) => skill.trim()); //trim() whitespace
    }
    ...
    try {
      ...
      if (profile) {
        ...
        profile = await Profile.findOneAndUpdate(
          ...
          { new: true } //to return doc after update was applied
        );

        return res.json(profile);
      }

      //create (to create if profile not found)
...
router.put(
  ...
  async (req, res) => {
    ...
    //create new object with data user submit
    const newExp = {
      title,
      company,
      location,
